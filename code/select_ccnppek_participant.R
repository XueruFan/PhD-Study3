...
for (s in steps) {
  ...
  sfc_avg <- sfc_sel %>%
    group_by(Site, Participant, Session) %>%
    summarise(
      across(starts_with("Net"), ~ mean(.x, na.rm = TRUE)),
      .groups = "drop"
    )
  ...
  merged <- sfc_avg %>%
    ...
    select(Site, Participant, Sex, Age, Session, starts_with("Net"))
  
  # 4. 输出
  write_xlsx(
    merged,
    file.path(sfc_dir, paste0("pek_step", s, "_fd0.3_sessionAvg.xlsx"))
  )
}
